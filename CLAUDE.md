...
- **Catalogue DEPTH is bounded by photography, and that is now a hard blocker.** Five to six products per shop is why the shop page's merchandising rows (C8) render nowhere — a rail needs to be a selection, not the catalogue printed twice. Growing the catalogue needs new product photos, and three things have to hold at once: a licence with no attribution requirement, a clean product shot, and **at least 1200×1200**, because seed-images.ts crops 1200² for angle 0 and 1560² for the zoom. Checked and ruled out: unsplash.com search now sits behind bot protection (401 "Making sure you're not a bot") and there is no API key in `.env`; the key-free alternative (api.openverse.org, `license=cc0,pdm`) fails all three — roughly 30% of results are usable at best and 6% for appliances (the rest are vintage advertisements, clip art and museum scans), its CC0 index contains at least one watermarked Dreamstime image, and every survivor across 56 candidates was 500–1024px on the long edge, i.e. below the crop size on the SHORT edge. Do not paper over this with upscaled or mixed-provenance imagery — it degrades the whole grid, which is the primary success criterion. It needs an Unsplash API key or client-supplied product photography.

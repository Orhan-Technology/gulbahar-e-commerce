...
import { Check, Eye, Heart, ImageOff, Pencil, X } from 'lucide-react';
...
import { bulkSetProductStatus, setProductStock } from '@/lib/actions/shop-products';
 ...
 */
function StockEditor({ productId, stock }: { productId: string; stock: number }) {
  const t = useTranslations('shopProducts');
  const locale = useLocale();
  const router = useRouter();
